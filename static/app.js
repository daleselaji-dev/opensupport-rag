...
function renderAnswerEval(summary) {
  ...
  const cases = (summary.cases || []).map((item) => {
    ...
    const reviewLabel = item.fallback_mode ? ` · ${item.fallback_mode}` : (item.needs_human_review ? ' · human_review' : ' · auto_answer');
    return `<details class="answer-case ${item.passed ? 'answer-case-pass' : 'answer-case-fail'}"><summary><strong>${text(item.case_id)}</strong><span>${item.passed ? 'PASS' : 'FAIL'} · ${text(item.expected_action)}${text(reviewLabel)} · ${text(item.latency_ms)} ms</span></summary><div class="answer-case-body"><p class="answer-question">${text(item.question)}</p><pre class="answer-text">${text(item.answer)}</pre><div class="answer-source-grid">${sourceCards || '<p class="hint">本条没有可展示的证据卡片。</p>'}</div><p class="answer-trace-line"><b>Trace：</b>${text(traceNames)}</p><p class="hint">citation_valid=${text(item.citation_valid)} · coverage=${text(item.citation_coverage)} · refusal=${text(item.refusal_signal)} · human_review=${text(item.needs_human_review)} · forbidden=${text((item.forbidden_claims_found || []).join(', ') || 'none')}</p></div></details>`;
  }).join('');
  answerEvalSummary.innerHTML = `<div class="eval-overview"><strong class="eval-result ${summary.overall_passed ? 'passed' : 'failed'}">${summary.overall_passed ? 'PASS' : 'FAIL'}</strong><span>${text(summary.benchmark_version)} · ${text(summary.case_count)} cases · p95 ${text(summary.metrics?.p95_ms)} ms</span></div><div class="gate-grid">${gates}</div><p class="hint">引用覆盖 ${text(summary.metrics?.citation_coverage)} · 拒答正确率 ${text(summary.metrics?.refusal_correctness)} · 危险声明 ${text(summary.metrics?.forbidden_claim_count)} · 生成错误/超时 ${text(summary.metrics?.answer_error_count)} · 可回答问题人工复核率 ${text(summary.metrics?.human_review_rate)} · grounded fallback ${text(summary.metrics?.grounded_fallback_count)}</p><details class="eval-details answer-cases-details"><summary>预览 ${text(summary.case_count)} 条生成回答、证据卡片和完整 Trace</summary><div class="answer-case-list">${cases}</div></details>`;
}
