...
def main() -> None:
    ...
    # Prefer the full Golden Draft answer run when it exists; keep the small
    # seed as a local-development fallback only.
    answer = read_json(ROOT / "reports" / "answer_eval_latest_v0_3.json") or read_json(ROOT / "reports" / "answer_eval_latest_v0_2.json")
    contextual = read_json(ROOT / "reports" / "eval_latest_v0_5_hybrid.json")
    stability = read_json(ROOT / "reports" / "stability_latest.json")
    security = read_json(ROOT / "reports" / "security_audit_latest.json")
    ...
    checks = [
        ...
        {"key": "production_unique_complaints", "actual": retrieval.get("index_inventory", {}).get("complaint_chunks", 0), "target": 10000, "passed": retrieval.get("index_inventory", {}).get("complaint_chunks", 0) >= 10000},
        ...
        {"key": "answer_p95_ms", "actual": answer.get("metrics", {}).get("p95_ms", "missing"), "target": 20000, "passed": isinstance(answer.get("metrics", {}).get("p95_ms"), (int, float)) and answer.get("metrics", {}).get("p95_ms") <= 20000},
        ...
        {"key": "security_unisolated_findings", "actual": {"private_pii": security.get("pii_findings", "missing"), "unisolated_prompt_injection": security.get("unisolated_prompt_injection_findings", security.get("prompt_injection_findings", "missing"))}, "target": {"private_pii": 0, "unisolated_prompt_injection": 0}, "passed": security.get("pii_findings") == 0 and security.get("unisolated_prompt_injection_findings", security.get("prompt_injection_findings")) == 0},
